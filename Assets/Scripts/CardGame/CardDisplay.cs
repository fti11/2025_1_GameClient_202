...
public class CardDisplay : MonoBehaviour
{
    ...
    public TextMeshPro desriptionText;          // 설명 텍스트 
    ...
    public void SetupCard(CardData data)
    {
        ...
        if (desriptionText != null) desriptionText.text = data.description;
        ...
        if(desriptionText != null)
        {
            desriptionText.text = data.description + data.GetAdditionalEffectsDescription();
        }
    }
    ...
    private void OnMouseUp()
    {
        ...
        else if (Physics.Raycast(ray, out hit, Mathf.Infinity, playerLayer))
        {
            if (playerStats != null)
            {
                if (cardData.cardType == CardData.CardType.Heal)        // 카드 효과에 따라
                {
        ...
        {
            if (cardManager != null)
                cardManager.DiscardCard(cardIndex);          // 효과가 없으면 바로 버리기
        }
    }

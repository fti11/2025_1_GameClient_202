...
public class JsonToScriptableConverter : EditorWindow
{
    ...
    [MenuItem("Tools/JASON to Scriptable Objects")]
        ...
        if (GUILayout.Button("Convert to Scriptable Objects"))
        {
            ...
            switch (conversionType)
            {
                case ConversionType.Items:
                    ConvertJasonToItemScriptableObjects();
                    break;
    ...
    private void ConvertJasonToItemScriptableObjects()                      // JSON 파일을 ScriptableObject 파일로 변환 시켜주는 함수
    {
        // 폴더 생성
        ...
        try
        {
            ...
            {
                ItemDatabaseSO database = ScriptableObject.CreateInstance<ItemDatabaseSO>();        // ItemDatabaseSO 셍성;
            ...
            EditorUtility.DisplayDialog("Succes", $"Created {createdItems.Count} scriptable objects!", "OK");
        ...
        try
        {
            ...
            foreach(var rowData in rowDataList)
            {
                ...
                if(!rowData.id.HasValue && !string.IsNullOrEmpty(rowData.choiceText)&& rowData.choiceNextId.HasValue)
                {
                    ...
                    {
                        Debug.LogWarning($"선택지 '{rowData.choiceText}'를 연결할 때 대화 (ID : {parentId})를 찾을 수 없습니다.");
                    }
                }
            }
